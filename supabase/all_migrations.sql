-- ================================================================
-- IIT ROPAR â€” FORMS DIGITALIZATION
-- SUPABASE (POSTGRESQL) SCHEMA  v2  â€” COMPLETE & AUDITED
-- ================================================================
--
-- FORMS COVERED
...
--
-- ALL STAKEHOLDERS
--   system_admin Â· student Â· faculty Â· tech_staff Â· non_tech_staff
--   project_staff (JRF/RA/Intern/Post-Doc/Temp) Â· supervisor Â· hod
--   student_affairs_ar Â· student_affairs_dr Â· student_affairs_jr
--   security_officer Â· hostel_warden Â· it_admin
--   forwarding_authority Â· deputy_registrar Â· registrar Â· dean_faa
--   guest_house_incharge Â· guest_house_committee_chair Â· approving_authority
--
-- TABLES (19 total)
...
CREATE TYPE user_role AS ENUM (
	...
	'student_affairs_ar',       -- AR â€” Student Affairs Section (signs F1)
	'student_affairs_dr',       -- DR â€” Student Affairs Section (signs F1)
	'student_affairs_jr',       -- JR(SA) â€” Student Affairs Section (signs F1)
	...
	'dean_faa',                 -- Dean FA&A â€” alternate final approver F4
...
-- ================================================================
-- SECTION 3 â€” CONFIGURATION TABLES
-- ================================================================
...
-- ================================================================
-- SECTION 6 â€” FORM-SPECIFIC DATA TABLES
-- ================================================================

-- ----------------------------------------------------------------
-- F1 Â· vehicle_sticker_data
-- APPLICATION FORM FOR ISSUE OF VEHICLE STICKER
-- For: Regular Student / Project Staff / JRF / Interns / Post Doc / RA / Others
--
-- Fields map:
--   1. Name             â†’ users.full_name
--   2. Designation      â†’ users.designation (Entry No / Emp No stored here)
--   3. Department       â†’ users.department
--   4. Address          â†’ address
--   5. Phone / e-mail   â†’ phone, email_contact
--   6. Driving License  â†’ driving_license_no, dl_valid_upto
--   7. Vehicle details  â†’ vehicle_details child table
--   8. Declaration      â†’ declaration_date (applicant name from users)
--   9. Supervisor rec   â†’ approval_stages stage 1 + 2 (with recommendation_text)
--  10. Hostel Mgmt      â†’ residing_in_hostel, hostel_verified_by/at
--  11. Student Affairs  â†’ approval_stages stage 4
--  12. Security Office  â†’ issued_sticker_no, sticker_valid_upto
-- ----------------------------------------------------------------
CREATE TABLE vehicle_sticker_data (
	id                  UUID    PRIMARY KEY DEFAULT gen_random_uuid(),
	submission_id       UUID    NOT NULL UNIQUE REFERENCES form_submissions(id) ON DELETE CASCADE,

	-- Fields 1â€“6 (name/dept/designation from users; stored as snapshot)
	...
	-- Section 12: Security Office â€” issued sticker
...
-- F1 Â· vehicle_details  (child of vehicle_sticker_data)
...
-- ----------------------------------------------------------------
-- F2 Â· email_id_request_data
-- REQUEST FORM FOR EMAIL ID AT IIT ROPAR
--
-- Sections:
--   Personal Details      â†’ initials, gender, first/last name, permanent_address
--   Employment Details    â†’ org_id, nature_of_engagement, role_in_org,
--                           department_section, project fields (if temp/project)
--   Alternate contact     â†’ mobile_no, alternate_email
--   Consent               â†’ consent_accepted_at
--   Office use (below dashes) â†’ assigned_email_id, date_of_creation,
...
CREATE TABLE email_id_request_data (
	...
	-- Consent (I have read email policiesâ€¦)
...
-- ----------------------------------------------------------------
-- F3 Â· hostel_undertaking_data
-- HOSTEL MANAGEMENT SECTION â€” INFORMATION CUM UNDERTAKING FORM
-- IIT ROPAR
--
-- Fields:
...
--   Emergency Contact No, Parents/Guardian address block â†’ guardian_details
...
CREATE TABLE hostel_undertaking_data (
	...
	-- Undertaking: "I have read the hostel rules of IIT Roparâ€¦"
...
-- F3 Â· guardian_details  (child of hostel_undertaking_data)
...
CREATE TABLE identity_card_data (
	...
	-- Fields 2â€“3 (employee_code from users; snapshot here)
...
-- ----------------------------------------------------------------
-- F5 Â· guest_house_data
-- GUEST HOUSE RESERVATION FORM â€” IIT ROPAR
--
-- Page 1 fields:
...
--   Applicant/Proposer table â†’ guest_house_proposers child table
--   Undertaking clauses (aâ€“f) â†’ undertaking_accepted_at
--   For Office Use Only â†’ room_no_confirmed, entry details, check-in/out
--
...
CREATE TABLE guest_house_data (
	...
	-- Field 5: Type of Occupancy (Single / Double â€” already in occupancy_type)

	-- Arrival and Departure (Fields â€” Arrival/Departure Information table)
	arrival_date              DATE            NOT NULL,
	arrival_time              TIME,
	departure_date            DATE            NOT NULL,
	departure_time            TIME,

	-- Fields 6â€“8: Purpose, Room type, Category
	...
	-- Undertaking clauses (aâ€“f) acceptance
...
-- ================================================================
-- SECTION 8 â€” ROW LEVEL SECURITY (RLS)
-- ================================================================
...
-- â”€â”€ role_assignments â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- â”€â”€ form_stage_config / form_access_config â€” read-only for all â”€
...
-- â”€â”€ form_submissions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- â”€â”€ approval_stages â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- â”€â”€ audit_log â€” admin read only â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- â”€â”€ form-specific data: submitter + assigned approver + admin â”€â”€
...
-- F2: Email ID Request â€” 2 stages
...
-- F3: Hostel Undertaking â€” 1 stage
...
-- F5: Guest House Reservation â€” 3 stages
...
-- F2: Email ID Request â€” any new joiner
...
-- F3: Hostel Undertaking â€” students only
('hostel_undertaking', 'student', TRUE, 'Students allotted hostel accommodation'),

-- F4: Identity Card â€” employees and faculty (not students, who get separate ID)
...
-- F5: Guest House â€” any registered user can request; approving authority validates category
...
-- ================================================================
-- SECTION 10 â€” DEFERRED FK (hostel parent signature)
...
-- ================================================================
-- END OF SCHEMA â€” iit_ropar_schema_v2.sql
-- 19 tables Â· 21 enums/types Â· full RLS Â· seeded config
-- ================================================================
...
-- Queue-wise delegation + temporary override support (idempotent)
