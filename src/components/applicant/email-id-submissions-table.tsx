...
type EmailIdSubmissionRow = {
  id: string;
  createdAt: string;
  statusText: string;
};

export function EmailIdSubmissionsTable({
  submissions,
}: {
  submissions: EmailIdSubmissionRow[];
}) {
        ...
        {submissions.map((submission) => {
          ...
          return (
            <Fragment key={submission.id}>
              <tr>
                <td className="px-5 py-3 font-medium text-gray-900">{submission.id}</td>
                    ...
                    <div className="space-y-3 rounded-xl border border-gray-200 bg-white p-3">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            const iframe = document.querySelector(`iframe[title="Email ID form ${submission.id}"]`) as HTMLIFrameElement;
                      ...
                      <iframe
                        src={`/forms/email-id/${submission.id}?embed=1`}
                        title={`Email ID form ${submission.id}`}
