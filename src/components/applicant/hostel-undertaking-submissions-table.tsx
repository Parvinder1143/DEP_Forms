...
type HostelUndertakingSubmissionRow = {
  submissionId: string;
  createdAt: string;
  statusText: string;
};

export function HostelUndertakingSubmissionsTable({
  submissions,
}: {
  submissions: HostelUndertakingSubmissionRow[];
}) {
        ...
        {submissions.map((submission) => {
          ...
          return (
            <Fragment key={submission.submissionId}>
              ...
              {isExpanded && (
                <tr>
                  <td className="bg-gray-50 px-5 py-4" colSpan={4}>
                    <div className="space-y-3 rounded-xl border border-gray-200 bg-white p-3">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            const iframe = document.querySelector(`iframe[title="Hostel undertaking form ${submission.submissionId}"]`) as HTMLIFrameElement;
                      ...
                      <iframe
                        src={`/forms/hostel-undertaking/${submission.submissionId}?embed=1`}
                        title={`Hostel undertaking form ${submission.submissionId}`}
