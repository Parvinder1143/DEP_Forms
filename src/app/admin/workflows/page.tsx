              ...
              <div className="space-y-2">
                {activeDelegations.slice(0, 6).map((delegation) => (
                  <p key={delegation.id} className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-900">
                    {delegation.delegatedRole}: {(delegation.requesterName ?? delegation.requesterEmail).trim()} -&gt; {(delegation.replacementName ?? delegation.replacementEmail).trim()} (till {new Date(delegation.endsAt).toLocaleDateString("en-IN")})
                  </p>
                ))}
