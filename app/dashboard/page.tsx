  ...
  const refreshApprovalNotifications = async () => {
    ...
    try {
      ...
      if (canReviewVehicleQueue) {
        setVehicleQueueItems(Array.isArray(queues.latestPendingVehicleForms) ? queues.latestPendingVehicleForms : [])
      }
      if (canReviewUndertakingQueue) {
        setUndertakingQueueItems(Array.isArray(queues.latestPendingUndertakingForms) ? queues.latestPendingUndertakingForms : [])
      }
    } catch {
      // Keep dashboard usable even if notification refresh fails.
    }
  }
          ...
          {canReviewAnyQueue && (
            <div className="relative">
              <button
                ...
                className="relative inline-flex items-center justify-center rounded-lg border border-gray-300 bg-white p-2.5 hover:bg-gray-50 transition"
              ...
              >
                ...
                {totalPendingApprovals > 0 && (
                  <span className="absolute -top-1 -right-1 min-w-[1.2rem] h-5 px-1 rounded-full bg-red-600 text-white text-[10px] font-bold flex items-center justify-center">
              ...
              {showNotificationPanel && (
                <div className="absolute right-0 mt-2 w-80 rounded-xl border border-gray-200 bg-white shadow-lg p-4 z-20">
                  <p className="text-sm font-semibold text-gray-900">Approval Notifications</p>
                  <p className="text-xs text-gray-500 mt-1">
                    {totalPendingApprovals > 0
                      ? `${totalPendingApprovals} form${totalPendingApprovals > 1 ? 's' : ''} awaiting approval/rejection`
                      : 'No pending approvals right now'}
                  </p>

                  <div className="mt-3 space-y-2">
                    {stakeholderNotificationItems.map((entry) => (
                      <div key={entry.key} className="flex items-center justify-between rounded-lg border border-gray-100 bg-gray-50 px-3 py-2">
                        <span className="text-xs text-gray-700">{entry.key}</span>
                              ...
                              {pendingEmailQueueItems.map((item) => (
                                <>
                                  <tr key={item.id} className="hover:bg-gray-50 align-top">
                                    ...
                                    <td className="px-3 py-2">
                                      <span className="text-xs font-semibold px-2 py-1 rounded-full bg-amber-50 text-amber-700 border border-amber-200">{item.status}</span>
                                    </td>
                                    ...
                                    <td className="px-3 py-2">
                                      ...
                                      >
                                        {expandedEmailDetails[item.id] ? 'Hide Form' : 'View Full Form'}
                                      </button>
                                    </td>
                                  </tr>
                              ...
                              {pendingIdentityQueueItems.map((item) => (
                                <>
                                  <tr key={item.id} className="hover:bg-gray-50 align-top">
                                    ...
                                    <td className="px-3 py-2 text-gray-700">{item.department_section || '-'}</td>
                                    <td className="px-3 py-2"><span className="text-xs font-semibold px-2 py-1 rounded-full bg-amber-50 text-amber-700 border border-amber-200">{item.status}</span></td>
                                    <td className="px-3 py-2 text-gray-600">{item.submitted_date ? new Date(item.submitted_date).toLocaleString() : 'No date'}</td>
                                    <td className="px-3 py-2">
                                      <button
                                        type="button"
                                        ...
                                        className="rounded-md border border-gray-300 bg-white px-2 py-1 text-xs font-semibold text-gray-700 hover:bg-gray-100"
                                      >
                                        {expandedIdentityDetails[item.id] ? 'Hide Form' : 'View Full Form'}
                                      </button>
                                    </td>
                                  </tr>
                                  ...
                                  {expandedVehicleDetails[item.id] && (
                                    <tr className="bg-gray-50">
                                      <td colSpan={7} className="px-3 py-3">
                                        ...
                                        <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">{item.stage_message || 'Approval is in progress.'}</div>
                                        ...
                                        {item.can_take_action && (
                                          <div className="mt-4">
                                            <label className="block text-xs font-medium text-gray-600 mb-1">Remark *</label>
                                            <textarea value={vehicleDecisionDrafts[item.id] || ''} onChange={(e) => setVehicleDecisionDrafts((prev) => ({ ...prev, [item.id]: e.target.value }))} rows={3} placeholder="Write decision remark visible to user" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-zinc-400 focus:border-transparent" />
                                          </div>
                                        )}

                                        {item.can_take_action ? (
                                          <div className="mt-4 flex flex-wrap gap-2">
                                            <button onClick={() => handleVehicleDecision(item.id, 'APPROVE')} disabled={processingVehicleId === item.id} className="px-4 py-2 rounded-lg bg-emerald-600 text-white text-sm font-medium hover:bg-emerald-700 disabled:opacity-60 transition">{processingVehicleId === item.id ? 'Processing...' : 'Accept'}</button>
                                            <button onClick={() => handleVehicleDecision(item.id, 'REJECT')} disabled={processingVehicleId === item.id} className="px-4 py-2 rounded-lg bg-red-600 text-white text-sm font-medium hover:bg-red-700 disabled:opacity-60 transition">{processingVehicleId === item.id ? 'Processing...' : 'Reject'}</button>
                                          </div>
                                        ) : (
                                          <p className="mt-4 text-sm text-gray-600">Awaiting action by another approval stage.</p>
                                        )}
                              ...
                              {pendingUndertakingQueueItems.map((item) => (
                                <>
                                  <tr key={item.id} className="hover:bg-gray-50 align-top">
                                    ...
                                    <td className="px-3 py-2 text-gray-700">{item.course_name || '-'}</td>
                                    <td className="px-3 py-2"><span className="text-xs font-semibold px-2 py-1 rounded-full bg-amber-50 text-amber-700 border border-amber-200">{item.status}</span></td>
                                    <td className="px-3 py-2 text-gray-600">{item.submitted_date ? new Date(item.submitted_date).toLocaleString() : 'No date'}</td>
                                    ...
                                    <tr className="bg-gray-50">
                                      <td colSpan={7} className="px-3 py-3">
                                        ...
                                        <div className="mt-4">
                                          ...
                                          <textarea value={undertakingRemarks[item.id] || ''} onChange={(e) => setUndertakingRemarks((prev) => ({ ...prev, [item.id]: e.target.value }))} rows={3} placeholder="Write decision remark visible to user" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-zinc-400 focus:border-transparent" />
                                        </div>
                                        <div className="mt-4 flex flex-wrap gap-2">
                                          <button onClick={() => handleUndertakingDecision(item.id, 'ACCEPT')} disabled={processingUndertakingId === item.id} className="px-4 py-2 rounded-lg bg-emerald-600 text-white text-sm font-medium hover:bg-emerald-700 disabled:opacity-60 transition">{processingUndertakingId === item.id ? 'Processing...' : 'Accept'}</button>
                                          <button onClick={() => handleUndertakingDecision(item.id, 'REJECT')} disabled={processingUndertakingId === item.id} className="px-4 py-2 rounded-lg bg-red-600 text-white text-sm font-medium hover:bg-red-700 disabled:opacity-60 transition">{processingUndertakingId === item.id ? 'Processing...' : 'Reject'}</button>
                                        </div>
                                      </td>
                                    </tr>
                                  )}
